...
{
    Vector2 size = MeasureTextEx(_font, tooltip.text, 14, 1.0f);
    float x = tooltip.x - (size.x / 2.0f);
    float y = tooltip.y - (size.y / 2.0f);

    DrawRectangleRounded({x - 3.0f, y + 1.0f, size.x + 8.0f, size.y + 8.0f},
                         2.0f,
                         6,
                         {0x00, 0x00, 0x00, 0x66});
...
void TooltipRenderer::Uninitialize() { UnloadFont(_font); }
