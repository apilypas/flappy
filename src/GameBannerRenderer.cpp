...
{
    _font = LoadFontEx("assets/fonts/golem-script.ttf",
                       (int)_textFontSize * 4,
                       0,
                       0);
}
...
void GameBannerRenderer::Render(GameBanner &b)
{
    ...
    DrawRectanglePro(box, {0, 0}, 0.0f, {0x00, 0x00, 0x00, 0xCC});

    this->DrawText(b.bannerText,
                   80 * b.scale,
                   box,
                   {0, -100.0f * b.scale},
                   {0xFF, 0xC9, 0x00, 0xFF});

    this->DrawText(b.authorText,
                   20 * b.scale,
                   box,
                   {0, 40.0f * b.scale},
                   {0xEF, 0xD9, 0x85, 0xFF});

    this->DrawText(b.hiScoreText,
                   20 * b.scale,
                   box,
                   {0, -260.0f * b.scale},
                   {0xFB, 0x8C, 0x81, 0xFF});

    this->DrawText(b.gameOverText,
                   _textFontSize * b.scale,
                   box,
                   {0, 140.0f * b.scale},
                   {0xFF, 0x42, 0x31, 0xFF});

    this->DrawText(b.text,
                   _textFontSize * b.scale,
                   box,
                   {0, 250.0f * b.scale},
                   b.color);
...
void GameBannerRenderer::Uninitialize() { UnloadFont(_font); }

void GameBannerRenderer::DrawText(const char *text,
                                  float fontSize,
                                  Rectangle box,
                                  Vector2 pos,
                                  Color color)
{
    ...
    auto size = MeasureTextEx(_font, text, fontSize, spacing);
